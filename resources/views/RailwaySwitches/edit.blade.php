  ...
  <div class="card2">
    <form action="{{route('stations.railway-switches.update', $station->id)}}" method="post">
      ...
      <div class="max-w-xl">
        ...
        <div class="grid grid-cols-9 font-semibold border-b divide-x bg-gray-50">
        ...
        <div class="grid grid-cols-9 border-0 border-b divide-x hover:bg-gray-100 hover:cursor-pointer ">
          ...
          <div class="col-span-6">
            <input type="text" name="switch[{{$key}}][name]" id="name-{{$key}}"
              class="w-full py-1 border-0 focus:ring-0 hover:cursor-pointer bg-inherit"
              value="{{$switch->name ? $switch->name : ' '}}">

          </div>
          <div class="flex items-center justify-center col-span-2 px-4 text-center ">
            <input type="checkbox" name="switch[{{$key}}][control]" class="w-4 h-4" value="1"
